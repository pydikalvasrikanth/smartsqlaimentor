...
const stages = [
  { name: "Driver", color: "#e25a1c", x: -4.5 },
  { name: "Cluster Manager", color: "#5382a1", x: 0 },
  { name: "Executors", color: "#a78bfa", x: 4.5 },
];
      ...
      {stages.map((s, i) => (
        <group key={s.name} position={[s.x, 0, 0]}>
          ...
          <Text position={[0, -1.5, 0]} fontSize={0.22} color="#e2e8f8">
            {i === 0 ? "Your PySpark script" : i === 1 ? "YARN · K8s · Standalone" : "Run tasks · hold partitions"}
          </Text>
        </group>
      ))}
      {[0, 1].map((i) => (
