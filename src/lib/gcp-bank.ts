...
export const GCP_BANK: GcpQuestion[] = [
  ...
  { id: 146, topic: "MySQL", difficulty: "beginner", companies: C_IND,
    question: "What is the difference between CHAR and VARCHAR in MySQL?",
    answer: "CHAR(n) is fixed-length: MySQL right-pads the value with spaces up to n characters and always stores n bytes (per charset). VARCHAR(n) is variable-length: stores actual length + 1 or 2 bytes overhead. CHAR is faster for truly fixed data (country codes, hashes); VARCHAR saves space for variable data (names, emails).",
    explanation: "Interviewer wants you to mention trailing-space handling (CHAR strips them on SELECT for non-BINARY, VARCHAR keeps them) and the storage overhead byte(s)." },
  { id: 147, topic: "MySQL", difficulty: "beginner", companies: C_IND,
    question: "Explain the different types of JOINs in SQL with examples.",
    answer: "INNER JOIN — rows matching in both tables. LEFT JOIN — all left rows + matching right (NULL if none). RIGHT JOIN — all right rows + matching left. FULL OUTER JOIN — all rows from both sides (MySQL emulates via UNION of LEFT + RIGHT). CROSS JOIN — Cartesian product. SELF JOIN — a table joined to itself, useful for hierarchies (employee → manager).",
    explanation: "Show ability to pick the right join: LEFT JOIN + WHERE right.id IS NULL is the classic anti-join pattern." },
  { id: 148, topic: "MySQL", difficulty: "beginner", companies: C_IND,
    question: "WHERE vs HAVING — when to use which?",
    answer: "WHERE filters rows BEFORE aggregation and cannot reference aggregate functions. HAVING filters AFTER GROUP BY on aggregated results and CAN reference aggregates like COUNT/SUM.\n\nExample: SELECT dept, COUNT(*) c FROM emp WHERE active = 1 GROUP BY dept HAVING COUNT(*) > 10;",
    explanation: "Use WHERE whenever possible — it cuts rows earlier and is index-friendly. HAVING only for post-aggregate filters." },
  { id: 149, topic: "MySQL", difficulty: "beginner", companies: C_IND,
    question: "Coding: Find the second-highest salary from an Employee(id, salary) table.",
    answer: "-- Method 1: LIMIT + OFFSET on distinct salaries\nSELECT DISTINCT salary\nFROM employee\nORDER BY salary DESC\nLIMIT 1 OFFSET 1;\n\n-- Method 2: Subquery (returns NULL when no 2nd)\nSELECT MAX(salary) AS second_highest\nFROM employee\nWHERE salary < (SELECT MAX(salary) FROM employee);\n\n-- Method 3: DENSE_RANK (MySQL 8+)\nSELECT salary FROM (\n  SELECT salary, DENSE_RANK() OVER (ORDER BY salary DESC) rk\n  FROM employee\n) t WHERE rk = 2 LIMIT 1;",
    explanation: "Functions used — DISTINCT (removes duplicate salaries so ties don't shift the offset), LIMIT/OFFSET (row window), MAX() (aggregate), DENSE_RANK() (window function that gives ties the same rank without gaps, unlike RANK())." },
  { id: 150, topic: "MySQL", difficulty: "beginner", companies: C_IND,
    question: "Coding: For each department, return the employee with the highest salary.",
    answer: "-- MySQL 8+: window function\nSELECT dept_id, emp_id, salary\nFROM (\n  SELECT dept_id, emp_id, salary,\n         ROW_NUMBER() OVER (PARTITION BY dept_id ORDER BY salary DESC) rn\n  FROM employee\n) t\nWHERE rn = 1;\n\n-- Pre-8 fallback: correlated subquery\nSELECT e.*\nFROM employee e\nWHERE e.salary = (SELECT MAX(salary) FROM employee WHERE dept_id = e.dept_id);",
    explanation: "ROW_NUMBER() gives a unique rank per partition — swap to RANK() if you want to keep ties. PARTITION BY resets the counter for each department; ORDER BY inside OVER() defines the ranking order." },
  { id: 151, topic: "MySQL", difficulty: "beginner", companies: C_IND,
    question: "What is a primary key, foreign key, and unique key? How do they differ?",
    answer: "PRIMARY KEY: uniquely identifies each row, cannot be NULL, one per table, auto-creates a clustered index in InnoDB. UNIQUE KEY: enforces uniqueness but allows one NULL (MySQL) and multiple uniques per table. FOREIGN KEY: enforces referential integrity — value must exist in the parent's primary/unique key. Supports ON DELETE / ON UPDATE actions (CASCADE, SET NULL, RESTRICT, NO ACTION).",
    explanation: "InnoDB stores rows physically in PK order (clustered index) — that's why choosing a small monotonic PK (BIGINT auto-increment or UUIDv7) matters for insert performance." },
  { id: 152, topic: "MySQL", difficulty: "intermediate", companies: [...C_IND, "Uber"],
    question: "Coding: Find users who logged in on 3 or more consecutive days from Logins(user_id, login_date).",
    answer: "SELECT DISTINCT user_id\nFROM (\n  SELECT user_id, login_date,\n         DATE_SUB(login_date,\n           INTERVAL ROW_NUMBER() OVER (PARTITION BY user_id ORDER BY login_date) DAY\n         ) AS grp\n  FROM (SELECT DISTINCT user_id, login_date FROM logins) d\n) g\nGROUP BY user_id, grp\nHAVING COUNT(*) >= 3;",
    explanation: "Classic 'gaps and islands' trick — if dates are consecutive, (date − row_number) is constant, so grouping by that difference collects each streak. Functions: ROW_NUMBER() (sequence per user), DATE_SUB / INTERVAL (subtract N days), COUNT(*) + HAVING (keep only streaks of ≥3)." },
  { id: 153, topic: "MySQL", difficulty: "intermediate", companies: C_IND,
    question: "Coding: Running total of daily sales per product.",
    answer: "SELECT product_id, sale_date, amount,\n       SUM(amount) OVER (\n         PARTITION BY product_id\n         ORDER BY sale_date\n         ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW\n       ) AS running_total\nFROM sales;",
    explanation: "Window frame ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW makes SUM cumulative. Without the frame clause, ORDER BY inside OVER defaults to RANGE which ties on equal dates — usually you want the explicit ROWS frame for a true row-by-row running total." },
  { id: 154, topic: "MySQL", difficulty: "intermediate", companies: [...C_IND, "Stripe"],
    question: "Coding: Find duplicate emails in Users(id, email) and delete duplicates keeping the smallest id.",
    answer: "-- Find duplicates\nSELECT email, COUNT(*) c\nFROM users\nGROUP BY email\nHAVING COUNT(*) > 1;\n\n-- Delete duplicates (keep smallest id)\nDELETE u1 FROM users u1\nJOIN users u2\n  ON u1.email = u2.email\n AND u1.id > u2.id;",
    explanation: "Self-join pattern: for each pair with the same email, delete the row with the larger id. Functions: COUNT(*) + HAVING for detection, multi-table DELETE syntax for surgical removal without a subquery (which MySQL disallows referencing the same table being deleted in older versions)." },
  { id: 155, topic: "MySQL", difficulty: "intermediate", companies: C_IND,
    question: "Difference between RANK(), DENSE_RANK() and ROW_NUMBER()?",
    answer: "ROW_NUMBER() — unique sequential number per partition (1,2,3,4). RANK() — same rank for ties, then skips (1,1,3,4). DENSE_RANK() — same rank for ties, no skip (1,1,2,3).\n\nSELECT name, salary,\n  ROW_NUMBER() OVER (ORDER BY salary DESC) rn,\n  RANK()       OVER (ORDER BY salary DESC) rk,\n  DENSE_RANK() OVER (ORDER BY salary DESC) drk\nFROM employee;",
    explanation: "Nth-highest salary is DENSE_RANK = N (ties treated as one). Top-N per group is ROW_NUMBER ≤ N (unique picks). RANK is for leaderboard-style scoring." },
  { id: 156, topic: "MySQL", difficulty: "intermediate", companies: C_IND,
    question: "Explain GROUP_CONCAT with an example and its limitations.",
    answer: "GROUP_CONCAT aggregates values from a group into a single delimited string.\n\nSELECT dept_id,\n       GROUP_CONCAT(DISTINCT emp_name ORDER BY emp_name SEPARATOR ', ') AS members\nFROM employee\nGROUP BY dept_id;\n\nLimits: output truncated at group_concat_max_len (default 1024 bytes). Fix: SET SESSION group_concat_max_len = 1000000;",
    explanation: "Interviewers love this because it tests knowledge of aggregate ordering, DISTINCT inside aggregate, custom separator, and the silent truncation pitfall." },
  { id: 157, topic: "MySQL", difficulty: "intermediate", companies: [...C_IND, "Amazon"],
    question: "Coding: Nth-highest salary as a reusable function.",
    answer: "CREATE FUNCTION getNthHighestSalary(N INT) RETURNS INT\nDETERMINISTIC\nBEGIN\n  DECLARE M INT;\n  SET M = N - 1;\n  RETURN (\n    SELECT DISTINCT salary\n    FROM employee\n    ORDER BY salary DESC\n    LIMIT 1 OFFSET M\n  );\nEND;",
    explanation: "LIMIT does NOT accept an expression directly in older MySQL — that's why we pre-compute M into a variable. DISTINCT ensures ties don't shift the offset. Marked DETERMINISTIC so the optimizer/replication can cache it." },
  { id: 158, topic: "MySQL", difficulty: "intermediate", companies: C_IND,
    question: "What are indexes? When does MySQL NOT use an index?",
    answer: "An index is a B-tree (or hash for MEMORY) structure that maps column values to row locations for fast lookup. MySQL skips or can't use an index when: (1) function applied to indexed column — WHERE YEAR(dt)=2024, (2) leading wildcard LIKE '%foo', (3) implicit type conversion (WHERE varchar_col = 123), (4) OR across different columns without a composite covering both, (5) low-selectivity column where full scan is cheaper, (6) index hint disables it, (7) query returns most of the table.",
    explanation: "Rewrite WHERE YEAR(dt)=2024 as WHERE dt >= '2024-01-01' AND dt < '2025-01-01' to keep it sargable — this is a top-tier interview question." },
  { id: 159, topic: "MySQL", difficulty: "intermediate", companies: C_IND,
    question: "Composite index leftmost-prefix rule — explain with example.",
    answer: "For INDEX(a, b, c), MySQL can use the index for filters on (a), (a, b), or (a, b, c) — but NOT for (b), (c), or (b, c) alone. The optimizer needs the leftmost columns to seek into the B-tree.\n\nSELECT * FROM t WHERE a = 1 AND b = 2;  -- uses index\nSELECT * FROM t WHERE b = 2;            -- cannot use index",
    explanation: "This is why column order in a composite index matters: put the most-filtered / equality columns first, range columns last." },
  { id: 160, topic: "MySQL", difficulty: "intermediate", companies: C_IND,
    question: "Explain the ACID properties in MySQL/InnoDB.",
    answer: "Atomicity — all statements in a transaction commit or none do (undo logs enable rollback). Consistency — DB moves from one valid state to another (constraints, FKs). Isolation — concurrent transactions don't see each other's partial work (MVCC + locks, controlled by isolation level). Durability — committed changes survive crashes (redo log + doublewrite buffer).",
    explanation: "Interviewers often follow up with 'which isolation level does MySQL default to?' — answer: REPEATABLE READ." },
  { id: 161, topic: "MySQL", difficulty: "intermediate", companies: [...C_IND, "Goldman Sachs"],
    question: "Transaction isolation levels — differences and phenomena prevented.",
    answer: "READ UNCOMMITTED — allows dirty reads. READ COMMITTED — prevents dirty reads, allows non-repeatable + phantom. REPEATABLE READ (InnoDB default) — snapshot per transaction; prevents dirty + non-repeatable; InnoDB also prevents phantom via gap locks. SERIALIZABLE — full range locking, prevents all three, effectively serial execution.",
    explanation: "Key nuance: unlike the standard, InnoDB's REPEATABLE READ blocks phantoms because of next-key locking on range scans." },
  { id: 162, topic: "MySQL", difficulty: "intermediate", companies: C_IND,
    question: "Coding: Pivot — count orders per user per status (PENDING/PAID/CANCELLED).",
    answer: "SELECT user_id,\n       SUM(CASE WHEN status='PENDING'   THEN 1 ELSE 0 END) AS pending,\n       SUM(CASE WHEN status='PAID'      THEN 1 ELSE 0 END) AS paid,\n       SUM(CASE WHEN status='CANCELLED' THEN 1 ELSE 0 END) AS cancelled\nFROM orders\nGROUP BY user_id;",
    explanation: "MySQL has no native PIVOT — use conditional SUM(CASE WHEN …). Prefer SUM over COUNT because COUNT(NULL)=0 but readability with SUM+1/0 is clearer, and both count rows correctly." },
  { id: 163, topic: "MySQL", difficulty: "intermediate", companies: C_IND,
    question: "COALESCE vs IFNULL vs NULLIF — when to use which?",
    answer: "COALESCE(a, b, c, ...) — SQL-standard, returns first non-NULL from any number of args. IFNULL(a, b) — MySQL-specific, exactly 2 args. NULLIF(a, b) — returns NULL if a=b else a; handy to avoid divide-by-zero: amount / NULLIF(qty, 0).",
    explanation: "Prefer COALESCE for portability. NULLIF combined with a division is the idiomatic way to avoid divide-by-zero errors in aggregate reports." },
  { id: 164, topic: "MySQL", difficulty: "advanced", companies: [...C_IND, "Uber", "Airbnb"],
    question: "Coding: Median salary per department.",
    answer: "-- MySQL 8+ using window functions\nSELECT dept_id, AVG(salary) AS median\nFROM (\n  SELECT dept_id, salary,\n         ROW_NUMBER() OVER (PARTITION BY dept_id ORDER BY salary) AS rn,\n         COUNT(*)     OVER (PARTITION BY dept_id)                 AS cnt\n  FROM employee\n) t\nWHERE rn IN (FLOOR((cnt+1)/2), CEIL((cnt+1)/2))\nGROUP BY dept_id;",
    explanation: "Median = average of the middle 1 or 2 rows depending on whether count is odd/even. FLOOR/CEIL of (cnt+1)/2 picks both positions cleanly. Functions: ROW_NUMBER() (position), COUNT() OVER (partition size without collapsing rows), AVG (combines the two middle values)." },
  { id: 165, topic: "MySQL", difficulty: "advanced", companies: [...C_IND, "Meta"],
    question: "Coding: Given Employee(id, manager_id, name), print the full manager chain for each employee.",
    answer: "WITH RECURSIVE chain AS (\n  SELECT id, name, manager_id, CAST(name AS CHAR(1000)) AS path, 1 AS depth\n  FROM employee\n  WHERE manager_id IS NULL\n  UNION ALL\n  SELECT e.id, e.name, e.manager_id,\n         CONCAT(c.path, ' > ', e.name), c.depth + 1\n  FROM employee e\n  JOIN chain c ON e.manager_id = c.id\n)\nSELECT id, name, path, depth FROM chain;",
    explanation: "Recursive CTE (MySQL 8+). Anchor member finds the roots; recursive member joins children. CAST is needed because concatenation would otherwise fix the column width to the anchor's length and truncate longer paths." },
  { id: 166, topic: "MySQL", difficulty: "advanced", companies: C_IND,
    question: "Coding: Detect gaps in a sequence — orders whose id has no predecessor.",
    answer: "-- Using LAG window function\nSELECT id\nFROM (\n  SELECT id, LAG(id) OVER (ORDER BY id) AS prev_id\n  FROM orders\n) t\nWHERE prev_id IS NULL OR id - prev_id > 1;",
    explanation: "LAG(col) returns the previous row's value in the partition. Detecting gaps = comparing current id to previous id. LEAD is the mirror function pointing forward." },
  { id: 167, topic: "MySQL", difficulty: "advanced", companies: C_IND,
    question: "What is a covering index? Why does it matter?",
    answer: "An index that contains ALL columns needed by the query — MySQL answers the query from the index alone without a table lookup (bookmark lookup). Shown as 'Using index' in EXPLAIN Extra column.\n\nINDEX(user_id, created_at, status)  -- covers: SELECT status FROM t WHERE user_id=? AND created_at BETWEEN ? AND ?",
    explanation: "Adding a rarely-filtered but frequently-selected column to the tail of an index can turn a slow query into an index-only scan — huge win for read-heavy tables." },
  { id: 168, topic: "MySQL", difficulty: "advanced", companies: [...C_IND, "Goldman Sachs"],
    question: "How do you read an EXPLAIN plan? Which access types are good vs bad?",
    answer: "Key columns: type (access method), possible_keys, key (chosen), rows (estimated), Extra. Access type ranking (best → worst): system > const > eq_ref > ref > range > index > ALL. Bad signs: type=ALL (full scan), Using filesort (sort without index), Using temporary (temp table), rows in millions when table is huge, key=NULL despite indexed column in WHERE.",
    explanation: "Use EXPLAIN ANALYZE (MySQL 8.0.18+) for actual timings rather than estimates. For JSON detail: EXPLAIN FORMAT=JSON." },
  { id: 169, topic: "MySQL", difficulty: "advanced", companies: [...C_IND, "Stripe"],
    question: "Coding: Sessionize events — group events per user into sessions where a new session starts after 30 minutes of inactivity.",
    answer: "SELECT user_id, event_ts,\n       SUM(new_session) OVER (PARTITION BY user_id ORDER BY event_ts) AS session_id\nFROM (\n  SELECT user_id, event_ts,\n         CASE\n           WHEN TIMESTAMPDIFF(MINUTE,\n                  LAG(event_ts) OVER (PARTITION BY user_id ORDER BY event_ts),\n                  event_ts) > 30\n             OR LAG(event_ts) OVER (PARTITION BY user_id ORDER BY event_ts) IS NULL\n           THEN 1 ELSE 0\n         END AS new_session\n  FROM events\n) t;",
    explanation: "Sessionization pattern — mark session boundaries with LAG + gap check, then running SUM turns the flag into a session id. Functions: LAG (previous event time), TIMESTAMPDIFF (gap in minutes), SUM() OVER (cumulative id assignment)." },
  { id: 170, topic: "MySQL", difficulty: "advanced", companies: C_IND,
    question: "Explain query optimization steps for a slow SELECT.",
    answer: "(1) Run EXPLAIN / EXPLAIN ANALYZE, identify full scans and filesort. (2) Add/reorder indexes; verify leftmost-prefix. (3) Rewrite non-sargable predicates (functions on columns, implicit casts). (4) Replace correlated subqueries with JOINs or CTEs. (5) Push filters before JOINs; use derived tables. (6) Check statistics: ANALYZE TABLE. (7) Look at buffer pool hit ratio (innodb_buffer_pool_size). (8) Partition huge tables by range on a hot filter column. (9) Cache with materialized results / summary tables. (10) Split OLTP vs analytics workload (read replica).",
    explanation: "Interviewers want a systematic approach, not a single silver-bullet answer. Always start with EXPLAIN, never guess." },
  { id: 171, topic: "MySQL", difficulty: "advanced", companies: C_IND,
    question: "Coding: Deduplicate keeping the latest row per key using ROW_NUMBER.",
    answer: "DELETE t FROM orders t\nJOIN (\n  SELECT id\n  FROM (\n    SELECT id,\n           ROW_NUMBER() OVER (PARTITION BY order_key ORDER BY updated_at DESC) rn\n    FROM orders\n  ) x\n  WHERE rn > 1\n) d ON t.id = d.id;",
    explanation: "Two-step: window-rank by newest per key, then delete every row where rn > 1. Wrapping in an additional derived table (x → d) sidesteps MySQL's 'can't modify the same table used in a subquery' restriction." },
  { id: 172, topic: "MySQL", difficulty: "advanced", companies: [...C_IND, "Meta"],
    question: "What is a CTE and how does it differ from a subquery / temp table?",
    answer: "A CTE (Common Table Expression) is a named temporary result set defined with WITH and referenced later in the same statement. Vs subquery: named, can be referenced multiple times, more readable. Vs temp table: no storage, lives only for the statement, no indexes. Recursive CTEs (WITH RECURSIVE) enable hierarchical / graph queries.",
    explanation: "MySQL 8 does NOT materialize non-recursive CTEs by default — they're inlined. If you rely on 'compute once, reuse many times' behavior, wrap in a temp table instead." },
  { id: 173, topic: "MySQL", difficulty: "advanced", companies: C_IND,
    question: "Coding: Top-3 selling products per category.",
    answer: "SELECT category, product_id, total_qty\nFROM (\n  SELECT category, product_id,\n         SUM(qty) AS total_qty,\n         DENSE_RANK() OVER (\n           PARTITION BY category\n           ORDER BY SUM(qty) DESC\n         ) AS rk\n  FROM sales\n  GROUP BY category, product_id\n) t\nWHERE rk <= 3;",
    explanation: "Window functions can be applied to aggregated columns — GROUP BY runs first, then DENSE_RANK ranks each category's products by total_qty. DENSE_RANK keeps ties (both #3 kept if tied)." },
  { id: 174, topic: "MySQL", difficulty: "professional", companies: [...C_FIN, "Uber"],
    question: "How do you handle deadlocks in MySQL/InnoDB?",
    answer: "Detect: SHOW ENGINE INNODB STATUS shows LATEST DETECTED DEADLOCK. InnoDB automatically rolls back the transaction with the smallest changed rows. Mitigations: (1) always access tables/rows in the same order across transactions, (2) keep transactions short, (3) use lower isolation (READ COMMITTED) if REPEATABLE READ's gap locks cause them, (4) add indexes so range locks don't cover extra rows, (5) retry rolled-back transactions in application code, (6) use SELECT … FOR UPDATE SKIP LOCKED for queue-like workloads.",
    explanation: "Application MUST retry deadlocked transactions — treating a deadlock as a fatal error is a common junior mistake." },
  { id: 175, topic: "MySQL", difficulty: "professional", companies: [...C_IND, "Amazon"],
    question: "How would you design and query a huge (500M rows) events table efficiently?",
    answer: "(1) Partition BY RANGE (TO_DAYS(event_ts)) with daily/monthly partitions — old partitions drop instantly (DROP PARTITION vs DELETE). (2) Composite PK / cluster key (event_ts, user_id, event_id) so range scans are sequential IO. (3) Secondary index on (user_id, event_ts) for user-timeline queries. (4) Archive cold partitions to object storage or a separate archive DB. (5) For analytics, replicate to columnar store (BigQuery / ClickHouse) — MySQL is OLTP, not OLAP. (6) Batch inserts + tune innodb_flush_log_at_trx_commit and innodb_buffer_pool_size.",
    explanation: "Interviewer wants the full lifecycle — write path, read path, retention, and analytics offload." },
  { id: 176, topic: "MySQL", difficulty: "professional", companies: C_IND,
    question: "Coding: Month-over-month growth % of revenue.",
    answer: "WITH monthly AS (\n  SELECT DATE_FORMAT(order_date, '%Y-%m-01') AS mth,\n         SUM(amount) AS revenue\n  FROM orders\n  GROUP BY 1\n)\nSELECT mth,\n       revenue,\n       LAG(revenue) OVER (ORDER BY mth) AS prev_revenue,\n       ROUND(\n         100.0 * (revenue - LAG(revenue) OVER (ORDER BY mth))\n         / NULLIF(LAG(revenue) OVER (ORDER BY mth), 0),\n         2\n       ) AS mom_growth_pct\nFROM monthly;",
    explanation: "DATE_FORMAT truncates to month start (portable equivalent of DATE_TRUNC). LAG pulls prior month. NULLIF guards divide-by-zero for the first row. Wrapping the LAG call in ROUND gives a clean percentage." },
  { id: 177, topic: "MySQL", difficulty: "professional", companies: [...C_FIN, "Stripe"],
    question: "How do you migrate a large MySQL table schema with zero downtime?",
    answer: "Use pt-online-schema-change or gh-ost: creates a shadow table with the new schema, copies rows in chunks, triggers/binlog stream keeps it in sync, then atomically renames. Alternatives: (1) Aurora / MySQL 8 native online DDL (ALGORITHM=INPLACE, LOCK=NONE) for supported changes. (2) Blue/green replicas with a controlled cutover. (3) Dual-write from the app to old + new columns during transition, then backfill.",
    explanation: "Senior expectation: pick the tool by change type — ADD COLUMN with default is INSTANT in 8.0+, but adding a UNIQUE index still needs gh-ost." },
  { id: 178, topic: "MySQL", difficulty: "professional", companies: [...C_IND, "Amazon"],
    question: "Explain replication in MySQL: async vs semi-sync vs group replication.",
    answer: "Async — primary commits and streams binlog; replicas may lag; risk of data loss on failover. Semi-sync — primary waits for at least one replica to ACK receipt of binlog before committing to client; bounded data loss. Group Replication (InnoDB Cluster) — Paxos-based, multi-primary or single-primary with automatic conflict detection and failover; the basis for MySQL InnoDB Cluster and Aurora-style HA.",
    explanation: "Senior answer mentions GTIDs (Global Transaction IDs) as the modern way to track and fail over replication safely — no more binlog position juggling." },
  { id: 179, topic: "MySQL", difficulty: "professional", companies: C_IND,
    question: "Common MySQL string / date functions every SQL engineer must know.",
    answer: "Strings: CONCAT, CONCAT_WS, SUBSTRING, LEFT/RIGHT, LENGTH (bytes) vs CHAR_LENGTH (chars), REPLACE, TRIM, LOWER/UPPER, LOCATE/INSTR, REGEXP_REPLACE (8+), JSON_EXTRACT / ->>.\n\nDates: NOW(), CURDATE(), DATE(dt), DATE_ADD/DATE_SUB, DATEDIFF (days), TIMESTAMPDIFF(unit, a, b), DATE_FORMAT(dt, '%Y-%m-%d'), STR_TO_DATE, WEEK, YEAR, MONTH, DAYOFWEEK, LAST_DAY.\n\nNulls: COALESCE, IFNULL, NULLIF.\nConditional: CASE WHEN, IF(cond, a, b).",
    explanation: "In interviews you'll be graded on picking the right function: LENGTH vs CHAR_LENGTH matters for multi-byte strings; DATEDIFF returns days only, so use TIMESTAMPDIFF for minutes/hours." },
  { id: 180, topic: "MySQL", difficulty: "professional", companies: [...C_IND, "Meta", "Uber"],
    question: "Coding: Retention — % of users who signed up in month M and returned in month M+1.",
    answer: "WITH signups AS (\n  SELECT user_id, DATE_FORMAT(MIN(event_ts), '%Y-%m-01') AS signup_mth\n  FROM events WHERE event_type='signup'\n  GROUP BY user_id\n),\nactivity AS (\n  SELECT DISTINCT user_id, DATE_FORMAT(event_ts, '%Y-%m-01') AS active_mth\n  FROM events\n)\nSELECT s.signup_mth,\n       COUNT(DISTINCT s.user_id) AS cohort_size,\n       COUNT(DISTINCT a.user_id) AS retained,\n       ROUND(100.0 * COUNT(DISTINCT a.user_id) / COUNT(DISTINCT s.user_id), 2) AS retention_pct\nFROM signups s\nLEFT JOIN activity a\n  ON a.user_id = s.user_id\n AND a.active_mth = DATE_ADD(s.signup_mth, INTERVAL 1 MONTH)\nGROUP BY s.signup_mth\nORDER BY s.signup_mth;",
    explanation: "Cohort analysis is a staple product-analytics interview question. Key ideas: build the cohort (signups CTE), define the return window (DATE_ADD +1 month), LEFT JOIN so users who didn't return still count in the denominator, then ratio the two DISTINCT counts." },
