// Server-only planner & mastery implementation. Holds all schemas, constants,
// helpers and handler bodies so that src/lib/plan.functions.ts stays a thin
// wrapper (Cloudflare server-fn splitting drops sibling module declarations).
...
export const InitPracticeInput = z.object({
...
export const NextPracticeInput = z.object({
  topic_slug: z.string().max(100),
...
export const RevealSolutionInput = z.object({ session_question_id: z.string().uuid() });
...
export const CompletePlanDayInput = z.object({ day_id: z.string().uuid() });

export const PlanFocusInput = z.object({ goal: z.string().min(2).max(2_000) });

export const AnalyzeFocusInput = z.object({
  topic_slug: z.string().max(100),
  goal: z.string().max(2_000),
});

// ---- GENERATE_PLAN ---------------------------------------------------------
export async function generatePlanImpl(
  data: z.infer<typeof GeneratePlanInput>,
...
// ---- GET_PLAN_STATE --------------------------------------------------------
export async function getPlanStateImpl(context: AuthedContext) {
  const { supabase, userId } = context;
  ...
  const today = (days ?? []).find((d: any) => d.day_index === dayNumber) ?? null;

  return { plan, days: days ?? [], today, day_number: dayNumber };
}

// ---- GET_LEARNING_STATE ----------------------------------------------------
export async function getLearningStateImpl(context: AuthedContext) {
  const { supabase, userId } = context;
  const { data: mastery } = await supabase.from("topic_mastery").select("*").eq("user_id", userId);
...
export async function initPracticeImpl(
  data: z.infer<typeof InitPracticeInput>,
  context: AuthedContext,
) {
...
export async function nextPracticeImpl(
  data: z.infer<typeof NextPracticeInput>,
  context: AuthedContext,
) {
  const { userId } = context;
  const res = await callWithRetry(
...
export async function revealSolutionImpl(
  data: z.infer<typeof RevealSolutionInput>,
  context: AuthedContext,
) {
  const { userId } = context;
...
export async function logAttemptImpl(
  data: z.infer<typeof LogAttemptInput>,
  context: AuthedContext,
) {
  const { supabase, userId } = context;
...
// ---- COMPLETE_DAY ----------------------------------------------------------
export async function completePlanDayImpl(
  data: z.infer<typeof CompletePlanDayInput>,
  context: AuthedContext,
) {
  const { supabase, userId } = context;
...
export async function planFocusImpl(data: z.infer<typeof PlanFocusInput>) {
  ...
  if (concepts.length === 0)
    return { error: "Couldn't derive concepts from that goal. Try rephrasing." };
...
export async function analyzeFocusImpl(
  data: z.infer<typeof AnalyzeFocusInput>,
  context: AuthedContext,
) {
  const { supabase, userId } = context;
  const { data: attempts } = await supabase
  ...
  const correct = list.filter((a: any) => a.is_correct).length;
  return {
    data: {
...
export async function awardPointsImpl(context: AuthedContext) {
  const { supabase, userId } = context;
...
export async function getProfilePointsImpl(context: AuthedContext) {
  const { supabase, userId } = context;
