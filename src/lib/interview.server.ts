...
export async function interviewTurnImpl(data: z.infer<typeof TurnInput>) {
...
export async function interviewTranscribeImpl(data: z.infer<typeof TranscribeInput>) {
    const apiKey = process.env.LOVABLE_API_KEY;
    if (!apiKey) return { error: "LOVABLE_API_KEY is not configured." };
...
export async function interviewSpeakImpl(data: z.infer<typeof SpeakInput>) {
    const apiKey = process.env.LOVABLE_API_KEY;
    if (!apiKey) return { error: "LOVABLE_API_KEY is not configured." };
...
export async function interviewReportImpl(data: z.infer<typeof ReportInput>) {
    const apiKey = process.env.LOVABLE_API_KEY;
    if (!apiKey) return { error: "LOVABLE_API_KEY is not configured." };
...
export async function interviewCorrectionsImpl(data: z.infer<typeof ReportInput>) {
    const apiKey = process.env.LOVABLE_API_KEY;
    if (!apiKey) return { error: "LOVABLE_API_KEY is not configured." };
    const transcript = data.history
      .map((t) => `${t.role === "interviewer" ? "INTERVIEWER" : "CANDIDATE"}: ${t.text}`)
