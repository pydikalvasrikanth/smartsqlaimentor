...
-- ============================================================
-- question_sessions: SERVER-ONLY answer-key store
-- Holds expected_sql and payload (python expected_solution / test_cases).
-- These answer keys must NEVER be readable by the browser, so the table is
-- accessible only via the service-role client inside auth-protected server
-- functions (which scope every query by user_id). No grants for anon /
-- authenticated and no client policy => all direct browser queries denied.
-- ============================================================
...
-- Intentionally NO policies and NO authenticated/anon grants: server-only table.